  ...
  var next = afterAll(function () {
    ...
    browser.on('up', function (s) {
      if (s.name === 'Foo Bar') {
        ...
        t.deepEqual(s.txt, {})
        t.deepEqual(s.rawTxt, new Buffer.from('00', 'hex'))
      } else {
        t.equal(s.name, 'Baz')
        t.equal(s.fqdn, 'Baz._test._tcp.local')
        t.deepEqual(s.txt, { foo: 'bar' })
        t.deepEqual(s.rawTxt, new Buffer.from('07666f6f3d626172', 'hex'))
      }
...
test('bonjour.find - binary txt', function (bonjour, t) {
  var next = afterAll(function () {
    ...
    browser.on('up', function (s) {
      t.equal(s.name, 'Foo')
      t.deepEqual(s.txt, { bar: new Buffer.from('buz') })
      t.deepEqual(s.rawTxt, new Buffer.from('076261723d62757a', 'hex'))
      bonjour.destroy()
      t.end()
    })
  })

  bonjour.publish({ name: 'Foo', type: 'test', port: 3000, txt: { bar: new Buffer.from('buz') } }).on('up', next())
})
